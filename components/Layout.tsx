...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, title }) => {
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 max-w-md mx-auto relative overflow-hidden shadow-2xl">
      ...
      <header className="bg-white/80 backdrop-blur-md px-6 pt-12 pb-4 flex justify-between items-center sticky top-0 z-20 border-b border-gray-100">
        <div>
          <h1 className="text-xl font-extrabold text-purple-600 tracking-tight">{title}</h1>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/95 backdrop-blur-lg border-t border-gray-100 px-6 pt-3 pb-8 flex justify-between items-center z-40">
        {NAVIGATION_ITEMS.map((item) => (
          ...
          >
            <div className={`transition-transform duration-200 ${
              item.id === 'add' 
                ? 'mb-6 -mt-12 bg-purple-600 text-white p-3 rounded-2xl shadow-xl shadow-purple-200 border-4 border-white' 
